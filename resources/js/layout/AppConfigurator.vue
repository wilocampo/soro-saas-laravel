...
const handleClickOutside = (event) => {
    if (configuratorRef.value && !configuratorRef.value.contains(event.target)) {
        // Don't close if clicking on other topbar actions
...
<template>
    <div ref="configuratorRef" id="config-panel" class="config-panel absolute top-[3.25rem] right-0 w-64 p-4 border rounded-lg origin-top shadow-[0px_3px_5px_rgba(0,0,0,0.02),0px_0px_2px_rgba(0,0,0,0.05),0px_1px_4px_rgba(0,0,0,0.08)] z-50 backdrop-blur-sm" :class="isDarkTheme ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-200'">
        <div class="flex flex-col gap-4">
            <div class="config-panel-colors">
                <div class="config-panel-label">Primary</div>
                <div>
                    <button
                        ...
                        :class="['border-none w-5 h-5 rounded-full p-0 cursor-pointer outline-none outline-offset-1', { 'active-color': layoutConfig.primary === primaryColor.name }]"
                        :style="{ backgroundColor: `${primaryColor.name === 'noir' ? 'var(--text-color)' : primaryColor.palette['500']}` }"
                    ></button>
                </div>
            </div>
            <div class="config-panel-colors">
                <div class="config-panel-label">Surface</div>
                <div>
                    <button
                        ...
                        :class="[
                            ...
                            { 'active-color': layoutConfig.surface ? layoutConfig.surface === surface.name : isDarkTheme ? surface.name === 'zinc' : surface.name === 'slate' }
                        ]"
            ...
            <div class="config-panel-settings">
                <div class="config-panel-label">Presets</div>
                <SelectButton v-model="preset" @change="onPresetChange" :options="presetOptions" :allowEmpty="false" />
            </div>
            <div class="config-panel-settings">
                <div class="config-panel-label">Menu Mode</div>
