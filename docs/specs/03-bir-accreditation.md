...
| Broker/agent commissions (brokers, entertainer agents) | **10% flat** (ind. & corp.) | **WI515 / WC515** | ◐ **draft-corrected 2026-07-24, reviewer to confirm** — earlier WI139/WI140, WC139/WC140 was wrong, and WI515 is a **flat 10%** code, not the 5% professional-fee rate. GENERIC commissions → professional fees WI010/WI011. Seed the full table from the eBIRForms ATC library (D25) |
...
- ~~**Retention period 5 vs 10 years**~~ ◐ **draft answer (2026-07-24 — D33):** the legal **floor is 5 years** (RR 7-2024); the pending-case extension is confirmed. **We keep 10 years as policy** and the legal-hold flag matches the rule. No change.
...
- ~~**VAT rounding method**~~ ◐ **draft answer (2026-07-24 — D24):** per-line half-up, VAT-inclusive backs out the net so VAT is the remainder. No issuance prescribes a method; **consistency is what is audited**, so the same policy must hold on screen, in the posting and in the return.
...
- ~~**⚠️ "Gapless" serial wording and "ACCN must appear on the face"**~~ ◐ **draft answer (2026-07-24 — D35):** "gapless" is **our** word, not BIR's — the rule is **sequential, unique, non-reusable**; keep the behaviour, drop the word from validation messages and product copy. **The ACCN IS required on the face** of issued documents (RMC 5-2021).
- **⚠️ "Ibex Global CTA Case No. 11075"** — docket/caption unverified; don't cite as authority.
- **Full current ATC↔rate mapping** — ◐ **draft answer, part-done (2026-07-24 — D25):** the seven drafted payment types are seeded, with commissions corrected to **WI515/WC515 at flat 10%**. The complete table still must be seeded from the current eBIRForms/eFPS ATC library, and both the seeded rates and the commissions scope are **reviewer-confirm items (draft-answers §4)** — **this blocks the withholding engine**.
- ~~**RR 31-2020 TWA thresholds/groupings**~~ ◐ **draft answer (2026-07-24 — D27):** the obligation starts the **1st of the month following publication**, and **de-listing likewise takes effect by publication** — hence `twa_effective_to` alongside `twa_effective_from`. Status is conferred by the published list, never computed.
