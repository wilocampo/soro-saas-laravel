...
- **⛔ Cash basis is unavailable to inventory-carrying tenants** (D38, draft answers 2026-07-24). This retired the Deferred COGS path (account 1450 and `CostOfSales`'s `deferred` branch), which existed only to serve that combination — now **removed**, enforced by the `StockLedger` guard.
